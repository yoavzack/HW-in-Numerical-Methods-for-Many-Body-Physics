...
<!-- To verify that it works, we will find the value of the largest value amplitude (can be seen easily in @fig-test-case):

```{julia}
#| label: optimal-state
max_coeff, max_index = findmax(abs.(ψ))
max_index = IndexToState(max_index, N)
print(max_index)
```

This is not exactly what we expected. --> We will calculate the energy of the ground state and of the output state. The function which calculates the energy is the following:
...
(ψ_opt_p, ψ_opt_m) = GenerateDimerState(N)
# print(2^N, " ", length(ψ), " ", length(ψ_opt), "\n")

iter_num = 30
E_rng = zeros(iter_num)
E_opt_p = zeros(iter_num)
E_opt_m = zeros(iter_num)
for ind in range(1, length=iter_num)
  ψ = MultiplyByHamiltonian(ψ, 1.0);
  E_rng[ind] = E(ψ)
  E_opt_p[ind] = E(ψ_opt_p)
  E_opt_m[ind] = E(ψ_opt_m)
end
plot(E_rng, label="Repeated Applications")
plot!(E_opt_p, label="Dimer State +")
plot!(E_opt_m, label="Dimer State -")
