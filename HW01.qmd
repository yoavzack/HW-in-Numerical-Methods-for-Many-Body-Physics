...
```{julia}
N = 6
ψDimer = dimer_state(N)
for ψ in ψDimer
  for (stateind, coeff) in enumerate(ψ)
    if abs(coeff) < 1e-9
      continue
    end
    # print(coeff, " ", index2state(stateind-1, N), "\n")
  end
  # print("\n")
end
```
...
#| output: false
function spinspin_correlation(ψ::Vector{<:Number}, x::Integer)
  @assert abs(norm(ψ) - 1) < 1e-9 "Input state is not normalized"
  D = length(ψ)
  @assert ispow2(D) "Length of wave-function must be a power of two"
  N = Int(log2(D))

  corr = 0
  for (stateind, coeff) in enumerate(ψ)
    state = 2 .* index2state(stateind, N) .- 1
    state_shifted = circshift(state, x)
    # print(coeff, "*", state, "⋅", state_shifted, "\n")
    corr += abs(coeff)^2 * (state' ⋅ state_shifted) / N
  end
  return corr
end
```
...
# 101010101... state
ψ = zeros(D)
flipstate = sum([2^(2*i) for i in range(0,length=Int(N/2))])
ψ[flipstate] = 1.0

# random state
# ψ = normalize(2 .* rand(rng, D) .- 1)

# random eigenstate
# ψ = zeros(D)
# ψ[rand(rng, 1:length(ψ))] = 1.0
...
# for ψ in ψDimer
  ...
  for x in range(0, length=N)
    # print(x, ":\n")
    corr[x+1] = spinspin_correlation(ψ, x)
  end
  plot!(corr)
# end
display(plt)
