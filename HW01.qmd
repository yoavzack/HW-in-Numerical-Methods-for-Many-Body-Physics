...
We start by creating a function to convert from the index of a state to the state itself, which we will use a lot later:
...
function multiply_heisenberg(ψ::Vector{<:Number}, J1::Real)
  ...
  for stateind in range(0, length=D)
    state = index2state(stateind, N) # <2>
    for (i, si) in enumerate(state) # <2>
      j = mod(i,N)+1 # <2>
      sj = state[j] # <2>
      if si == sj
...
In order to calculate the energy of the ground state, we need to create it first. The ground state for $N$ spins in a Heisenberg chain is Ferromagnetic for $J<0$ and Antiferromagnetic for $J>0$, i.e. the Néel State. So the function which generates the ground state is the following:
...
for ind in range(1, length=iternum)
  ...
  ENeel[ind] = heisenberg_energy(ψNeel, J)
end
...
This is not exactly a linear relation, but it is about linear. I guess this is the best we can hope for in small system sizes. #TODO

# Question 4: Spin-Spin Correlations

We first write a function which takes a wavefunction $\ket{\psi}$ and a basis $b$ and returns the spin-spin correlation $\left< S_i S_{1+x} \right>$:
