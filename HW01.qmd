...
$$
H = J_1 \sum_{i} \vec{S}_{i} \cdot \vec{S}_{i+1} = J_{1} \sum_{\left<i,j\right>} \left[\frac{1}{4}(\sigma^+_i \sigma^-_j + \sigma^-_i \sigma^+_j) +\frac{1}{2} \sigma^z_i \sigma^z_j \right]
$$

This means the for each pair of adjacent spins $i,j$, the state $\ket{\psi}$ turns into a sum of two elements:

1. A state with switched places for all pairs of opposite spins, with coefficient $\frac{J_1}{4}$.
2. The same state as $\ket{\psi}$, with a coefficient $\frac{J_1}{2}$ and also a sum over all pairs of adjacent spins, $+1$ for ++/-- and $-1$ for +-/-+.
...
function MultiplyByHamiltonian(ψ::Vector{Float64}, J1::Float64)
  ...
  for stateInd in range(0, length=D)
    ...
    for (i, si) in enumerate(state)
      ...
      sj = state[j]*2-1 # <2>
      if si == sj
        ψ_out[stateInd+1] += J1/4*coeff # <3>
      else
        ψ_out[stateInd+1] -= J1/4*coeff # <3>
        stateIndNew = FlipSpins(stateInd, i, j) # <4>
        ψ_out[stateIndNew+1] += J1/2*coeff # <4>
      end
    end
  end
  ψ_out = normalize!(ψ_out) # <5>
  return ψ_out
end
```
...
function DimerState(N::Int)
  ...
  for stateInd in range(0, length=2^Int(N/2)) # <2>
    pairs = IndexToState(stateInd, Int(N/2))
    ...
    index_m = bitarr_to_int(circshift(IndexToState(index_p, N), 1)) # <5>
...
(ψ_Dimer_p, ψ_Dimer_m) = DimerState(N)
plot(ψ_Dimer_p, label="Dimer Dimer +")
plot!(ψ_Dimer_m, label="Dimer State -")
```

Which is remarkably similar to the results of the [repeated application](#fig-repeated-test) of the Hamiltonian. Let's plot the difference between them:

```{julia}
#| label: fig-dimer-repeated-difference
plot(ψ .- ψ_Dimer_p, label="Difference from Dimer +")
plot!(ψ .- ψ_Dimer_m, label="Difference from Dimer -")
```
...
function NeelState(N::Int, J::Float64)
...
for ind in range(1, length=iter_num)
  ψ = MultiplyByHamiltonian(ψ, 1.0);
  E_rng[ind] = E(ψ)
  E_Dimer_p[ind] = E(ψ_Dimer_p)
  E_Dimer_m[ind] = E(ψ_Dimer_m)
  E_Neel[ind] = E(ψ_Neel)
end
...
But why is this below the Dimer states? AAAAA
