...
for (Nind, N) in enumerate(Narr)
  ...
  for (gind, g) in enumerate(garr)
    ...
    res = eigs(H; nev=2, which=:SR, ritzvec=false)[1]
    E0[Nind, gind] = res[1]
    E1[Nind, gind] = res[2]
  end
end
...
plt = plot()
for (gind, g) in enumerate(garr)
  plot!(1 ./ Narr, ΔE[:, gind], marker=:circle, label=@sprintf("g = %.3f", g) )
end
xlabel!("1/N")
ylabel!("ΔE")
display(plt)
...
plot(1 ./ Narr, ΔE[:,3], marker=:circle, yaxis=:log, label=nothing)
xlabel!("1/N")
ylabel!("ΔE")
```

This is not exactly a linear relation, but it is about linear. I guess this is the best we can hope for in small system sizes.

# Question 4: Spin-Spin Correlations

To calculate the spin-spin correlation of a the ground state, we first need the ground state. We already have the Néel State which is the ground state for $g < g_c$, but for $g>g_c$ we need to generate Dimer States, using the following function:
...
function dimer_state(N::Integer)
  @assert N>0 "Number of spins must be positive"
  @assert mod(N,2)==0 "Only even number of spins are supported"

  D = 2^N
  ψ_p = zeros(D)
  ψ_m = zeros(D)
  for stateInd in range(0, length=2^Int(N/2))
    ...
    index_m = bitarr2int(circshift(index2state(index_p, N), 1))
    ψ_p[index_p+1] += sign
    ψ_m[index_m+1] += sign
  end
...
Now that we have the states, we can calculate the spin-spin correlation:
...
#| fig-cap: "The spin-spin correlation $\braket{S_{i} | S_{i+x}}$ for different $x$ values, in a log-log plot."
#| layout-ncol: 2
#| layout-nrow: 2
#| fig-subcap:
#|   - "loglog plot for $g=0$"
...
for (gind, g) in enumerate(garr)
  ...
  ψ = normalize(rand(rng, length(b)))
  ψ_old = normalize(rand(rng, length(b)))
  while abs(abs(ψ_old' ⋅ ψ) - 1.0) > sqrt((1e-6)^2*length(ψ))
    ψ_old = ψ
    ψ = normalize(H*ψ)
  end

  for x in range(0, length=Int(N/2))
    corr[gind, x+1] = spinspin_correlation(ψ, b, N, x)
  end
  plt = plot(corr[gind, :].^2, label=@sprintf("g = %1.3f", g), marker=:circle)
  plot!(yaxis=:log, size=(250,250))
  if g <= gc
    plot!(xaxis=:log)
  end
  display(plt)
...
We can see that indeed the correlation decays as a power law for $g \leq g_c$ and exponentially for $g > g_c$. for $g = 1/2$ we get a constant almost-zero correlation which I don't understand yet.
...
function bondbond_correlation(ψ::Vector{<:Number}, b::fixed_sz_basis, N::Integer, x::Integer)
  ...
  mean2 = 0
  for (substateind, coeff) in enumerate(ψ)
    if mod(substateind, 2) == 0 # <1>
      continue # <1>
    end # <1>

    stateind1 = b.states[substateind]
    state = 2 .* index2state(stateind1, N) .- 1
    bond1 = abs(coeff)^2 * state[1] * state[2]
    bond2 = abs(coeff)^2 * state[mod(x,N)+1] * state[mod(x+1,N)+1]
...
for (gind, g) in enumerate(garr)
  ...
  plot!(corr[gind, :].^2, label=@sprintf("g = %1.3f", g), marker=:circle)
end
display(plt)
```