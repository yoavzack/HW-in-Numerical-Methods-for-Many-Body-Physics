...
Nmax = 16 # runs up to Nmax=16. Nmax=18 is a bit too much for my computers.
```
...
#| fig-cap: "The spin-spin correlation for different $x$ abd $g$ values"
#| layout-ncol: 2
#| layout-nrow: 2
#| fig-subcap:
#|   - "log-log plot for $g=0$"
#|   - "log-log plot for $g=g_c$"
#|   - "semilogy plot for $g=0.49$"
...
for (gind, g) in enumerate(garr)
  ...
  plt = plot(xarr.+1, corr[gind, :].^2, label=@sprintf("g = %1.3f", g), 
    marker=:circle, xlabel="x", ylabel="Correlation") # <3>
...
#| fig-cap: "The bond-bond correlation for different $x$ and $g$ values"
#| layout-ncol: 2
#| layout-nrow: 2
#| fig-subcap:
#|   - "log-log plot for $g=0$"
#|   - "log-log plot for $g=g_c$"
#|   - "log-log plot for $g=0.49$"
#|   - "log-log plot for $g=0.5$"
N = Nmax
D = 2^N
J1 = 1.0
...
for (gind, g) in enumerate(garr)
  ...
  plt = plot(xarr .+ 1, corr[gind, :].^2, label=@sprintf("g = %1.3f", g), marker=:circle, xlabel="x", ylabel="Correlation") # <3>
  plot!(xaxis=:log, yaxis=:log, size=(250,200)) # <3>
  display(plt) # <3>
end
```
1. Generate constants $S_z$ basis and Hamiltonian
2. Get the ground state by `Arpack`
3. Calculate and plot Correlations in the ground state
